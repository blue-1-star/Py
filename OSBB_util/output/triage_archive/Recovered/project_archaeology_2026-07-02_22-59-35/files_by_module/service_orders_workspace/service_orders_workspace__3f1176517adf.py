...
def _offer_buttons(state: dict, lang: str) -> list[list[str]]:
    ...
    for offer in _current_offers():
        ...
        if profile in PROFILE_TO_KEY and profile not in known:
            ...
            extra.append(offer)
    for profile in ("REMOTE_REPROGRAM_OWN", NEW_REMOTE_PROFILE, "REMOTE_REFURBISHED_FROM_STOCK", "PHONE_ACCESS_CONNECT"):
...
def _interest_card(interest: dict, lang: str) -> str:
    ...
    return "\n".join([
        ...
        f"Кількість: {interest.get('quantity')}",
...
async def show_resident_services(update: Update, user_states: dict, user_id: int, *, lang: str) -> None:
    data = _account_and_unit(user_id)
    if not data:
        await update.message.reply_text(tr(lang, "need_unit"), reply_markup=kb([[tr(lang, "back_portal")], [HOME]]))
        return
    try:
    ...
    state.clear()
    state.update({"_module": MODULE, "area": "resident", "mode": "resident_home", "account": data["account"], "unit": data["unit"]})
    body = f"{tr(lang, 'title')}\n\n{tr(lang, 'intro')}"
    if not _current_offers():
        body += "\n\n" + tr(lang, "no_offers")
    await update.message.reply_text(body, reply_markup=kb(_offer_buttons(state, lang)))
...
async def _show_preview(update: Update, state: dict, lang: str) -> None:
    ...
    if price is None:
        await update.message.reply_text("⚠️ Для услуги не задана цена.", reply_markup=kb([[BACK], [HOME]]))
        return
    ...
    await update.message.reply_text(
        "\n".join([
            ...
            f"Кількість: {quantity}",
            f"Вартість: {money(total)} {offer.get('currency') or 'UAH'}", "",
            "До підтвердженої оплати це лише намір. Після оплати створюється справжнє замовлення.",
        ]),
        reply_markup=kb([[tr(lang, "create_interest")], [tr(lang, "cancel"), BACK], [HOME]]),
    )
...
async def _handle_resident(update: Update, user_states: dict, user_id: int, message_text: str, *, lang: str, state: dict) -> bool:
    ...
    if mode == "resident_home":
        ...
        if offer:
            state["offer"] = offer
            if text(offer.get("workflow_profile_code")) == NEW_REMOTE_PROFILE:
...
async def show_service_operator_workspace(update: Update, user_states: dict, user_id: int, *, lang: str) -> None:
    ...
    await update.message.reply_text(
        ...
        reply_markup=kb([[tr(lang, "open_orders")], [tr(lang, "supplier_demand"), tr(lang, "supplier_batches")], [tr(lang, "refresh")], [HOME]]),
    )
...
async def _handle_operator(update: Update, user_states: dict, user_id: int, message_text: str, *, lang: str, state: dict) -> bool:
    ...
    if _operator_back(message_text):
        await show_service_operator_workspace(update, user_states, user_id, lang=lang); return True
    mode = text(state.get("mode"))
    ...
    if mode == "operator_order_card":
        ...
        if action == "activate_phone":
            ...
            await update.message.reply_text(tr(lang, "phone_prompt"), reply_markup=kb([["⬅️ К оператору"], [HOME]])); return True
        try:
...
async def handle_service_orders_text(update: Update, user_states: dict, user_id: int, message_text: str, *, lang: str, user_mode: str | None) -> bool:
    ...
    if user_mode == "client" and message_text in _resident_entry_texts():
        await show_resident_services(update, user_states, user_id, lang=lang)
        return True
    if user_mode == "admin" and message_text in {"🔑 Заявки на пульты", "📞 Телефонный доступ"} and has_service_workspace_access(user_id):
        await show_service_operator_workspace(update, user_states, user_id, lang=lang)
        return True
    return False
