            ...
            critical.append(
                {
                    "code": "PARKING_TIME",
                    "vehicle_id": vehicle.get("id"),
                    "text": f"Для {label} не вказано тариф/статус паркування (parking_time).",
                }
            )
...
def profile_snapshot(
    *,
    ...
    if confirmed_needed and not is_currently_confirmed and not any(
        item["code"] == "PENDING_OPERATOR" for item in critical
    ):
        critical.append(
            {
                "code": "PROFILE_CONFIRMATION",
                "text": "Потрібно підтвердити правильність показаних даних.",
            }
        )
    ...
    if not critical and is_currently_confirmed:
    ...
    return {
        ...
        "phone_access_allowed": not critical and is_currently_confirmed,
...
def confirm_profile(
    *,
    ...
    structural = [
        item for item in snapshot["critical"]
        if item["code"] != "PROFILE_CONFIRMATION"
    ]
    if structural:
        raise ValueError(
            "Неможливо підтвердити профіль: критичні дані ще не заповнені."
        )
