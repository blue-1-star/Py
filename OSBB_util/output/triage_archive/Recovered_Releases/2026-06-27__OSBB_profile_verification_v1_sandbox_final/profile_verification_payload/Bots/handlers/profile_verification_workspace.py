...
BUTTON_CONFIRM = {
    "ru": "✅ Подтвердить: данные правильные",
    "uk": "✅ Підтвердити: дані правильні",
    "en": "✅ Confirm: data is correct",
}
BUTTON_NO_CAR = {
...
def _vehicle_lines(snapshot: dict) -> list[str]:
    ...
    if not vehicles:
        ...
        return ["🚗 Автомобілі: ⚠️ у базі записів немає; потрібне підтвердження"]
    lines = ["🚗 Автомобілі:"]
    for i, vehicle in enumerate(vehicles, 1):
        lines += [
            f"{i}. Номер: {_fmt(vehicle.get('plate'))}",
            f"   Марка/модель: {_fmt(vehicle.get('model'))}",
            f"   Колір: {_fmt(vehicle.get('color'))}",
            f"   Паркування: {_fmt(vehicle.get('parking_time'))}",
        ]
    return lines
...
def _profile_card(snapshot: dict, lang: str) -> str:
    ...
    lines = [
        "📋 Перевірка даних",
        "",
        f"🏠 Квартира: {profile.get('apartment_number') or '⚠️ не вказано'}",
        *_vehicle_lines(snapshot),
        "",
    ]

    critical = snapshot.get("critical") or []
    advisory = snapshot.get("advisory") or []
    if critical:
        lines += ["⚠️ Критично — необхідно завершити до підключення телефонного доступу:"]
        lines += [f"• {item.get('text')}" for item in critical]
    else:
        lines += ["✅ Критичні дані заповнені."]
    if advisory:
        lines += ["", "ℹ️ Також перевірте:"]
        lines += [f"• {item.get('text')}" for item in advisory]

    status = text(profile.get("verification_status"))
    lines += [
        "",
        f"Статус перевірки: {status}",
        "",
...
def _gate_message(snapshot: dict, lang: str) -> str:
    ...
    lines += [
        "",
        "Після підтвердження коректних даних телефонний доступ стане доступним.",
        "",
        "Номер для відкриття може бути окремим і не збігається з контактними телефонами.",
    ]
    return "\n".join(lines)
...
async def show_profile_verification(
    ...
    *,
    ...
    rows: list[list[str]] = []
    critical_codes = {text(item.get("code")) for item in snapshot.get("critical") or []}
        ...
        if state.get("area") == "resident":
            ...
            if state.get("mode") == "profile_card":
                ...
                if message_text == _tr(BUTTON_CONFIRM, lang):
                    ...
                    try:
                        ...
                        conn.close()
                    await update.message.reply_text("✅ Дані підтверджено.")
                    await show_profile_verification(update, user_states, user_id, lang=lang)
                    return True
            ...
            if state.get("mode") == "parking_time_choice":
                ...
                await update.message.reply_text(
                    ...
                    "До її рішення телефонний доступ недоступний."
                )
                await show_profile_verification(update, user_states, user_id, lang=lang)
                return True
