...
async def apply_edit_value(
    ...
    try:
        if action == "area_sqm":
            ...
            old, new = update_unit_field(unit_id, "area_sqm", new_value, user_id)

        elif action in {"display_name", "official_number", "internal_note"}:
            old, new = update_unit_field(unit_id, action, new_value, user_id)

        elif action in {"contact_name", "contact_phone"}:
            old, new = update_primary_contact(unit_id, action, new_value, user_id)
...
async def set_unit_status(
    ...
    old, new = update_unit_field(unit_id, "record_status", new_status, user_id)
    await update.message.reply_text(
        f"✅ Статус изменён\n\nБыло: {status_label(old)}\nСтало: {status_label(new)}"
    )
    await show_unit_card(update, user_states, user_id, unit_id)
