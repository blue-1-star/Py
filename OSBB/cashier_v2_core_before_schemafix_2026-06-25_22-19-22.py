...
def normalize_period(value: Any, *, required: bool = False) -> str | None:
    ...
    if raw in {"", "-", "не указан"}:
        if required:
            raise ValueError("Укажите период: ГГГГ-ММ, например 2026-07.")
        return None
...
def open_charges(
    *,
    ...
    try:
        ...
        acol = allocation_amount_column(cur)
        ...
        if apartment_id is not None and "apartment_id" in ccols:
            where.append("c.apartment_id = ?")
            params.append(int(apartment_id))
        elif apartment_number and "apartment_number" in ccols:
            where.append("c.apartment_number = ?")
        ...
        if entrance_number:
            ...
            if entrance_col and "apartment_id" in ccols:
                where.append(f"a.{entrance_col} = ?")
        ...
        status_sql = (
            ...
            if "charge_status" in ccols else (
                "AND COALESCE(c.status, '') <> 'cancelled'"
                if "status" in ccols else ""
            )
        )
        apt_join = "LEFT JOIN apartments a ON a.id = c.apartment_id" if entrance_number else ""
        unit_expr = (
            "COALESCE(a.apartment_number, c.apartment_number)"
            if entrance_number and "apartment_number" in ccols
            else "c.apartment_number" if "apartment_number" in ccols
            else "NULL"
        )
        ...
        cur.execute(
            ...
            {status_sql}
            GROUP BY c.id
            HAVING c.amount - COALESCE(SUM(pa.{acol}), 0) > 0.00001
            ORDER BY {unit_expr}, c.id
            """,
            tuple(params),
        )
        result = []
