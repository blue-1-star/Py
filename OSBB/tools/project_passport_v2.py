...
# -*- coding: utf-8 -*-
"""
OSBB Project Passport v2 - READ ONLY / SOURCE FOCUSED

Назначение:
- строит паспорт именно исходного кода проекта, а не Data/exports и не БД;
- не читает и не выгружает SQLite .db;
- не читает Excel/архивы/картинки;
- пишет только отчеты в OSBB/Data/exports/code_passport/<timestamp>/.

Отличие v2:
- исключает Data/exports, Data/db, Data/logs, Data/raw, Data/backups;
- исключает ранее созданные отчеты, чтобы повторный запуск не "раздувал" паспорт;
- suppresses SyntaxWarning при ast.parse старых файлов с Windows-путями в docstring.
"""
...
EXCLUDED_DIRS = {
    ...
    # OSBB generated / data areas. We are building a code passport, not a data passport.
    "exports",
    "logs",
    "db",
    "backups",
...
# Directory paths to exclude by OSBB-relative prefix.
...
def is_excluded(path: Path, root: Path) -> bool:
    ...
    if path.is_file():
        ...
        if suffix in EXCLUDED_FILE_SUFFIXES:
            return True
        # extra protection for sqlite sidecar names
...
def main() -> int:
    ...
    out_dir = root / "Data" / "exports" / "code_passport" / f"project_passport_v2_{stamp}"
    ...
    summary = {
        ...
        "root": str(root),
        "out_dir": str(out_dir),
        "scope": "source-focused; Data/exports, Data/db, Data/logs, Data/raw excluded",
    ...
    largest_modules = sorted(modules, key=lambda x: x.lines, reverse=True)[:30]
    most_functions = sorted(modules, key=lambda x: x.functions, reverse=True)[:30]
    ...
    body.append(f"- Scope: {summary['scope']}")
    ...
    if parse_errors:
        ...
        body.append("(none)")
    write_md(out_dir / "00_Project.md", "OSBB Project Passport v2", "\n".join(body))
    ...
    for d in sorted(by_dir):
        ...
        for f in sorted(by_dir[d], key=lambda x: x.path.lower())[:250]:
    ...
    print("OSBB Project Passport v2 - READ ONLY")
    print(f"Root: {root}")
    print(f"Output: {out_dir}")
    print("Scope: source-focused; Data/exports, Data/db, Data/logs, Data/raw excluded")
