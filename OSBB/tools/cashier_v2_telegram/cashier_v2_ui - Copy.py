...
async def ask_payer(update: Update, user_states: dict[int, Any], user_id: int, group: str, service: dict | None = None) -> None:
    ...
    await update.message.reply_text(
        "Введите квартиру или цифры госномера.\nНапример: 98 или 3804.",
...
async def choose_service_screen(update: Update, user_states: dict[int, Any], user_id: int, group: str) -> None:
    ...
    await update.message.reply_text("Услуга:", reply_markup=service_kb(options))
...
async def handle_cashier_v2_text(update: Update, context: ContextTypes.DEFAULT_TYPE, user_states: dict[int, Any], user_id: int) -> bool:
    ...
    if text == BTN_NEXT:
        ...
        if subject_mode == 'commercial':
            ...
            await update.message.reply_text(
                "Введите фирму, помещение или договор.",
                reply_markup=kb([[BTN_BACK, BTN_MAIN]]),
            )
            return True
    ...
    if text == BTN_SETTINGS:
        await update.message.reply_text("⚙️ Настройки кассы пока не изменяются здесь.", reply_markup=menu_kb()); return True
    ...
    if state.get('screen') == 'commercial_query':
        ...
        await update.message.reply_text("Выберите фирму:", reply_markup=payer_kb(items))
        return True
    ...
    if state.get('screen') == 'payer_query_first':
        ...
        await update.message.reply_text("Выберите плательщика:", reply_markup=payer_kb(items)); return True
    ...
    if state.get('screen') == 'card':
        ...
        await update.message.reply_text("Выберите действие.", reply_markup=card_kb(draft)); return True
    ...
    if state.get('screen') == 'edit_menu':
        ...
        if text == BTN_EDIT_COMMENT:
            ...
            await update.message.reply_text("Комментарий:", reply_markup=kb([[BTN_SKIP_COMMENT],[BTN_BACK_TO_CARD,BTN_CANCEL]])); return True
