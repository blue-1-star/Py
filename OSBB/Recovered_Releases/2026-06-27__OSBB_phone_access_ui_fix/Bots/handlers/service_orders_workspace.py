...
def _is_phone_offer(offer: dict | None) -> bool:
    return text((offer or {}).get("workflow_profile_code")) == "PHONE_ACCESS_CONNECT"
...
def _offer_buttons(state: dict, lang: str, *, section: str = "all") -> list[list[str]]:
    """Build a clear resident menu without exposing duplicate/internal offers."""
    ...
    for offer in _current_offers():
        ...
        if section == "phone" and profile != "PHONE_ACCESS_CONNECT":
            continue
        if section == "remote" and profile == "PHONE_ACCESS_CONNECT":
            continue
        # A duplicate test/technical service must not appear as a second
        # resident-facing button for the same workflow.
...
def _order_card(order: dict, *, title: str) -> str:
    ...
    return "\n".join([
        ...
        *([f"Телефон доступу: {_requested_phone(order)}"] if _requested_phone(order) else []),
...
def _interest_card(interest: dict, lang: str) -> str:
    ...
    return "\n".join([
        ...
        *([f"Телефон доступу: {_requested_phone(interest)}"] if _requested_phone(interest) else []),
...
async def _show_preview(update: Update, state: dict, lang: str) -> None:
    ...
    if is_phone:
        lines.append(f"{tr(lang, 'phone_number')}: {state.get('requested_phone') or '-'}")
    else:
...
def _create_interest_from_state(state: dict, user_id: int) -> dict:
    ...
    if _is_phone_offer(state.get("offer")):
        ...
        # Structured value survives the interest -> paid-order promotion.
        resident_comment = f"ACCESS_PHONE={phone}"
    ...
    return create_service_interest(
        ...
        quantity=int(state.get("quantity") or 1),
        resident_comment=resident_comment,
    )
