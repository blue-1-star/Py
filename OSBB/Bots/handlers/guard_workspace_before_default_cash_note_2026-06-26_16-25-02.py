...
async def _save_manual_cash(
    ...
    if not note:
        await update.message.reply_text(
            "Нужно указать основание: например «передано лично, кв.174, за июль»."
        )
        return
...
async def handle_guard_workspace_text(
    ...
    *,
    ...
    if mode == "manual_cash_amount":
        ...
        await update.message.reply_text(
            "Введите основание: например «передано лично, за июль».\n"
    ...
    if mode == "manual_cash_note":
        state["note"] = "" if message_text == "-" else message_text
    ...
    if mode == "manual_cash_review":
        ...
        if message_text == "✏️ Сумма":
            ...
            return True
        if message_text == "📝 Основание":
            state["mode"] = "manual_cash_note"
            await update.message.reply_text("Введите новое основание.", reply_markup=kb([[BACK], [HOME]]))
            return True
