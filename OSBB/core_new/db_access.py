...
# Этот файл живёт в core_new/db_access.py — корень проекта на один уровень выше.
CORE_NEW_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = CORE_NEW_DIR.parent
...
def get_conn():
    # В OSBB_cl только одна боевая база (paths.DB_FILE) — без переключателя
    # USE_TEST_DB/OSBB_TEST_DB_FILE/OSBB_DB_FILE, который был в старом OSBB.
