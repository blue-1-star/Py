...
def is_full_ua_like_plate(plate):
    p = normalize_plate(plate)
    return re.fullmatch(r"[A-Z]{1,3}\d{4}[A-Z]{1,3}", p) is not None


def one_extra_digit_variant(better, worse):
    """
    KA9552TB beats KA95552TB:
    the worse variant has one extra duplicated/manual digit.
    """
    b = normalize_plate(better)
    w = normalize_plate(worse)

    bd = digits_only(b)
    wd = digits_only(w)

    if len(bd) != 4 or len(wd) != 5:
        return False

    can_reduce = any(wd[:i] + wd[i + 1:] == bd for i in range(len(wd)))

    if not can_reduce:
        return False

    b_letters = re.sub(r"\d", "", b)
    w_letters = re.sub(r"\d", "", w)

    return b_letters == w_letters or SequenceMatcher(None, b_letters, w_letters).ratio() >= 0.80


def is_corrupted_short_paper_variant(better, worse):
    """
    AA12354BT beats AA54B:
    the worse value is too short and looks like a broken paper record.
    """
    b = normalize_plate(better)
    w = normalize_plate(worse)
    ...
    if not (is_full_ua_like_plate(b) or is_full_ua_plate(b)):
        return False

    wd = digits_only(w)
    bd = digits_only(b)

    if wd and not all(ch in bd for ch in wd):
        return False

    wl = set(re.sub(r"[^A-Z]", "", w))
    bl = set(re.sub(r"[^A-Z]", "", b))

    return bool(wl & bl)


def is_better_structural_plate(candidate, other):
    """
    V4 structural correction:
    - full plate supported by shortened/incomplete variant;
    - 4-digit plate beats a 5-digit duplicate/manual typo;
    - complete plate beats heavily corrupted short paper value.
    """
    return (
        is_full_plate_supported_by_shorter(candidate, other)
        or one_extra_digit_variant(candidate, other)
        or is_corrupted_short_paper_variant(candidate, other)
    )
...
def summarize_group(group):
    ...
    for row in vote_rows:
        ...
        for other in vote_rows:
            ...
            if is_better_structural_plate(plate, other_plate):
                row["effective_votes"] += 1
                row["rule_notes"].append(
                    f"bonus: {other_plate} structurally supports {plate}"
                )
                other["rule_notes"].append(
                    f"weakened: likely incomplete/corrupted variant of {plate}"
                )
    ...
    if not top:
        ...
        if top["effective_votes"] > top["votes"]:
            recommendation_reason = "номер предпочтительнее: другой вариант похож на усечённую, неполную или повреждённую запись"
        else:
