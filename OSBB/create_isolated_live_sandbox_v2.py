r"""
Создание следующей изолированной sandbox-копии для живого теста.
...
Что делает:
- сохраняет исторический базовый остаток и существующие операции osbb_test.db
  как стартовый снимок; не трактует их как ошибку;
...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-nonempty-base",
        action="store_true",
        help="Совместимость: исторические операции теперь разрешены по умолчанию.",
    )
    ...
    # osbb_test.db — не пустой шаблон, а базовый тестовый контур с
    # историческими кассовыми операциями. Их нельзя автоматически объявлять
    # «мусором» и отбрасывать. Новая sandbox будет изолированной копией
    # этого состояния: новые живые тесты начнутся после момента создания
    # снимка, а исходная БД останется неизменной.
    inherited = {name: count for name, count in counts.items() if count > 0}
    ...
    lines = [
        "=" * 100,
        "ISOLATED LIVE SANDBOX CREATION",
        "=" * 100,
        f"Base test DB (read-only): {base}",
        f"New sandbox: {target}",
        f"Guard user: {args.guard_user or '(not assigned)'}",
        "",
        "Baseline workflow counts inherited from osbb_test.db:",
        *[f"  {name}: {count}" for name, count in counts.items()],
        "",
        "Important: non-zero baseline values are inherited history, not a failure.",
        "",
    ]
    ...
    try:
        ...
        lines.extend([
            ...
            *[f"   {name}: {count}" for name, count in output_counts.items()],
            ...
            "",
            "RESULT: READY FOR ISOLATED LIVE SANDBOX TEST",
            "Historical base operations were preserved as a baseline; "
            "new test operations begin only in this new sandbox.",
