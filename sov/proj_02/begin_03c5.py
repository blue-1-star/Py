...
for i, day in enumerate(unique_days):
    ...
    for j, source in enumerate(sources):
        ...
        axes[0].bar(x + j * width, means, width, label=f'{source} - Day {day}', color=colors[j], alpha=0.7)

# Добавляем подписи для каждого дня
...
for i, day in enumerate(unique_days):
    ...
    for j, factor in enumerate(factors):
        ...
        axes[1].bar(x + j * width, means, width, label=f'{factor} - Day {day}', color=colors[j], alpha=0.7)
