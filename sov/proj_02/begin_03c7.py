...
for i, day in enumerate(unique_days):
    ...
    for j, source in enumerate(sources):
        ...
        if i == 0:
            axes[0].text(x[0] + j * width, means.max(), f'Day {day}', ha='center', va='bottom')
...
axes[0].legend(title='Source', bbox_to_anchor=(1, 1), loc='upper left')
...
for i, day in enumerate(unique_days):
    ...
    for j, factor in enumerate(factors):
        ...
        # Добавляем подписи для каждого дня только на первый график
        if i == 0:
            axes[1].text(x[0] + j * width, means.max(), f'Day {day}', ha='center', va='bottom')
...
# Уменьшаем количество элементов в легенде
axes[1].legend(title='Factor', bbox_to_anchor=(1, 1), loc='upper left')
