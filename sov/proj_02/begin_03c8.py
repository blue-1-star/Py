...
for i, day in enumerate(unique_days):
    ...
    axes[0].text(np.mean(x) + width * (len(sources) / 2 - 0.5), means.max() + 0.05, f'Day {day}', ha='center', va='bottom')

# Добавляем подписи для факторов на оси X
axes[0].set_title('Влияние факторов на источники')
axes[0].set_xticks(np.arange(len(factors) * len(unique_days)) + width * (len(sources) / 2 - 0.5))
...
for i, day in enumerate(unique_days):
    ...
    # Добавляем метки для каждого дня
    axes[1].text(np.mean(x) + width * (len(factors) / 2 - 0.5), means.max() + 0.05, f'Day {day}', ha='center', va='bottom')

# Добавляем подписи для источников на оси X
axes[1].set_title('Влияние источников на факторы')
axes[1].set_xticks(np.arange(len(sources) * len(unique_days)) + width * (len(factors) / 2 - 0.5))
