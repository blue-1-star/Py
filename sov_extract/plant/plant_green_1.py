...
def getting_pixel_counts(img_path, lower_green=(35, 50, 50), upper_green=(85, 255, 255), show_images=False):
    """
    Вычисляет количество зелёных пикселей на изображении с учетом пропорций при ресайзе.
    
    :param img_path: Путь к изображению.
    :param lower_green: Нижний порог для зелёного цвета в формате (H, S, V).
    :param upper_green: Верхний порог для зелёного цвета в формате (H, S, V).
    :param show_images: Показывать ли изображения (для отладки).
    :return: Количество зелёных пикселей, общее количество пикселей и коэффициент масштабирования.
    """
    img = cv.imread(img_path)
    if img is None:
        sys.exit("Could not read image")
    ...
    base_resize = 500
    new_width = base_resize
    new_height = int((base_resize * height) / width)
    ...
    return white_pixel_count, total_pixels, scale_factor
...
# ----------------------------------
# 
def visualize_green_contours(img_path, output_folder, lower_green=(35, 50, 50), upper_green=(85, 255, 255)):
    """
...
def generate_excel_file(folder_path, excel_file_name, scale_data):
    """
    Генерирует Excel-файл с данными о площади зелёных листьев с учетом коэффициента масштабирования.
    ...
    for cur_img_path in image_files:
        ...
        # Получаем данные о зелёных пикселях и коэффициент масштабирования
        white_pixels, total_pixels, scale_factor = getting_pixel_counts(cur_img_path, show_images=False)
        
        # Корректируем масштаб с учетом коэффициента масштабирования
        adjusted_scale_pix_per_mm = scale_pix_per_mm * scale_factor
        
        # Вычисляем процент зелёных пикселей
        white_percent = (white_pixels / total_pixels) * 100
        white_percent_rounded = round(white_percent, 2)
        
        # Вычисляем площадь зелёных листьев в мм^2 с учетом скорректированного масштаба
        white_pixels_area_mm = round((white_pixels / (adjusted_scale_pix_per_mm ** 2)), 2)
        
        # Записываем данные в строку
        row_data = [
            file_name, sample_name, img_date, img_time, adjusted_scale_pix_per_mm,
            white_pixels, total_pixels, f"{white_percent_rounded}%", white_pixels_area_mm
        ]
