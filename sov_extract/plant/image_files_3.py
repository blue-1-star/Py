...
def list_files_to_excel(folder_path, file_pattern="*", excel_file_name="file_list.xlsx"):
    ...
    моделью камеры и фокусным расстоянием.
    ...
    # Заголовки таблицы
    headers = ["File Name", "File Size (MB)", "Width (px)", "Height (px)", "Scale pix/mm", "Cam", "Focal Length (mm)"]
    ...
    # Записываем имена файлов (без расширения), их размеры, размеры изображений, модель камеры и фокусное расстояние в Excel
    for row_num, file_name in enumerate(files, start=1):
        ...
        # Получаем размеры изображения (ширину и высоту), модель камеры и фокусное расстояние
        try:
