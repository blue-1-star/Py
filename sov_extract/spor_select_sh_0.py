...
def process_directory(directory, mode='rectangle', excel_file='results.xlsx'):
    ...
    (отсутствует файл с суффиксом _cont_sel.png), вызывается функция select_shapes.
    Результаты для всех файлов накапливаются и добавляются в Excel-файл.
    При сохранении в Excel в столбце "Image" указывается только базовое имя файла.
